...
- **Ditto (product):** The knowledge lifecycle meta-process (Insight-042) should be designed as distributed maintenance + centralised audit, not as a single knowledge-manager agent. Every process that reads memories/context should be able to flag staleness. The learning layer (L5) aggregates these signals.
