...
**Absorption criterion:** this insight stays `active` until at least **two** user-facing seams have applied the "Ditto-as-X" default explicitly and shipped. Brief 200 is application #1. A second application (likely a legibility sub-brief for outbound/inbound/generated artefacts that considers "Ditto-as-object-store" vs S3/R2) is needed before absorption into `docs/architecture.md`.
