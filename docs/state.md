...
- **Reviewer flag (Brief 072):** `form-submit` actions bypass the session-scoped action registry validation. Follow-up: register form-submit actions when blocks are emitted, or add alternative session validation.
- **Reviewer flag (Brief 074):** `matchTaskToProcessFromList` uses substring slug matching — may produce false positives for short slugs. Follow-up: switch to word-boundary or exact token matching.
