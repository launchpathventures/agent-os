...
**Follow-up (2026-04-21, post-close):** `/autobuild` Step 6 tightened from single-pass P0/P1 fix to iterative loop (max 3 attempts + non-convergence guard) mirroring `/drain-queue` Step F's shape. Per-pass: fix → full re-verify → fresh-subagent `/dev-reviewer` + fresh-subagent `/dev-review` (maker-checker preserved by spawning new subagents each pass). Escalates to human with pass count + failure mode + branch name; brief stays `in_progress` and no PR opens. Cost ceiling rises from 1× to up-to-3× pre-PR reviewer spend (most briefs exit pass 1 with zero P0/P1 from initial review; only messy briefs pay the multiplier). §Constraint "/autobuild Step 4/5 MUST spawn NEW agents" still satisfied — the tightening adds re-spawn passes, doesn't inline. Direct edit to `.catalyst/skills/autobuild/SKILL.md` Step 6 + Failure Modes table; no schema/code changes elsewhere. Trigger: user request "tighten loop with autobuild" 2026-04-21.
