# Research: Human Cognition Models for Ditto
...
- Key insight for Ditto: System 2 observes System 1's output and only intervenes when the output is considered to infringe on rational decision-making rules. This is structurally analogous to the harness monitoring agent output — the harness IS System 2 for the agent's System 1.
...
Ditto's implicit feedback (edits-as-diffs) captures **articulated** corrections — the human already identified the problem. But Rob's "that doesn't look right" and Lisa's "that's not us" are pre-articulate. The system needs a vocabulary for vague signals. Potential approaches:
...
| Level | Verb | What it means | Ditto analogy |
|-------|------|---------------|------------------|
...
- **SAE Level 3 autonomous vehicles:** The system knows when it's out of its depth and requests human takeover. Ditto's confidence metadata (ADR-011) is this pattern.
...
Ditto has trust tiers (frequency of review) and attention modes (form of review). Missing: **stakes awareness** — how much cognitive effort is this output worth? A process could declare a stakes profile:
...
| Ditto (current) | No — process-scoped + agent-scoped flat memory | No | No |

### Ditto Relevance

Ditto's memory architecture (ADR-003) has two scopes: agent-scoped and process-scoped. Neither models entities, relationships, or temporal validity. When Rob reviews a quote for Henderson, the system doesn't know:
...
- **Ditto relevance:** ACT-R's activation-based memory retrieval validates Ditto's existing memory salience scoring (`confidence × log(reinforcement+1) × recency_decay` from context-and-token-efficiency research). The production rule system maps to process step execution — but ACT-R models the cognitive WHY behind rule selection, not just the execution.
...
- **Ditto relevance:** Soar's impasse-driven learning is structurally analogous to Ditto's correction-to-improvement pipeline. When the agent "gets stuck" (output rejected), it creates a subgoal (understand the correction), resolves it (apply the fix), and stores the result (process memory). But Soar does this automatically within the cognitive cycle, while Ditto requires human intervention. The gap is the same: how to move from concrete correction to compiled knowledge.
...
- **Ditto relevance:** LIDA's attention mechanism is the most directly relevant to Ditto's attention model (ADR-011). LIDA models what "deserves attention" computationally — attention codelets compete, the most salient wins. Ditto's confidence-based routing is a simplified version: low confidence = demands attention. But LIDA's model is richer — salience depends on novelty, urgency, personal relevance, and emotional valence, not just confidence.
...
Across all research areas, seven distinct cognitive dimensions emerge that are relevant to how humans think through work. Each dimension is currently absent or undermodeled in Ditto:
...
## Gap Analysis: Ditto Architecture
...
|-----------|-----------|-------------|-----------------|
| Cognitive Mode | No direct source. Bloom's Taxonomy provides the level classification. | Bloom's cognitive levels are well-established in education. No AI platform applies them to review UX. | Cognitive mode as a process/step declaration that adapts review experience — Original to Ditto. |
| Expertise Level | Dreyfus model provides the framework. | ACT-R's activation-based memory partially models expertise. No agent platform tracks human expertise. | Per-process human expertise tracking integrated with trust — Original to Ditto. |
| Tacit Knowledge | Knowledge elicitation literature (Springer 2022). | Industry 4.0 tacit knowledge elicitation via cooperative games + ontologies. No agent platform attempts this. | Structured rejection vocabulary + elicitation prompts in review flow — Original to Ditto. |
| Abstraction Level | Weick's sensemaking. Soar's impasse-driven learning. | Soar automatically compiles from problem-solving experience. No agent platform escalates corrections to insights. | Insight escalation engine (correction → pattern → structural → strategic) — Original to Ditto. |
| Challenge Orientation | Edmondson's psychological safety framework. SAE Level 3 self-assessment. | ADR-011 confidence scoring is the embryo. Claude's safety refusal is the closest AI example. | Agent challenge function for work-quality pushback — Original to Ditto. |
| Stakes Awareness | Simon's satisficing. Cognitive Load Theory (Springer 2026). | Content moderation uses three-band confidence. No agent platform uses stakes profiles. | Stakes-aware review depth with adaptive interface density — Original to Ditto. |
